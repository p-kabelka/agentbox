...
The OPA server runs on a dedicated `opa-net` network (`internal: true`), shared only with the proxy. The agent container is **not** on `opa-net` and cannot reach OPA at the network level — DNS resolution for `opa` fails and no IP route exists. This is enforced by Podman's network namespace isolation, not by environment variables or proxy settings, so it cannot be bypassed by an agent with root access. The proxy is on all three networks (`agent-net`, `proxy-net`, `opa-net`) and is the only container that can reach both the agent and OPA. No TLS is needed for the proxy-to-OPA hop since `opa-net` is internal with no external exposure.
...
The OPA server runs as a sidecar container on a dedicated internal network. The following changes are added to `compose-base.yaml`:

```yaml
networks:
  agent-net:
    internal: true   # no internet route; enforced by Podman
  ...
  opa:
    ...
    volumes:
      - type: bind
        ...
        read_only: true
    networks:
      - opa-net      # only opa-net — not agent-net, not proxy-net
    restart: unless-stopped
```

The OPA container is on `opa-net` only. The agent container is on `agent-net` only. These are separate Podman networks with no shared connectivity — the agent cannot reach OPA even with root access (no IP route, no DNS resolution). The proxy is on all three networks and is the sole bridge between them. OPA does not need internet access — policies are loaded from a read-only bind-mounted volume.

Key flags:

- `--server`: Run as a daemon with REST API.
- `--addr=0.0.0.0:8181`: Listen on all interfaces (`opa-net` only — no external exposure).
...
```
policies/
  main.rego           # Entry point: default-deny, deny-overrides-allow logic, preset allowlist
  services.rego       # User-authored rules for external services (Jira, K8s, GitHub, etc.)
  managed.rego        # Auto-generated by agentbox allow/deny CLI (per-host allow and deny rules)
  data.json           # Optional: external data (allowed ticket IDs, namespace prefixes, etc.)
```

| File | Who writes it | Content |
|------|--------------|---------|
| `main.rego` | Preset | Default `allow := false`, deny-overrides-allow logic, preset allowlist rules (package registries, mirrors, documentation sites) |
| `managed.rego` | `agentbox allow`/`deny` CLI | Auto-generated per-host `allow` and `deny` rules. Simple one-liners only. |
| `services.rego` | User | Custom policies for external services with body inspection (Jira ticket filtering, K8s namespace scoping, etc.) |
| `data.json` | User | External data referenced by Rego policies (allowed ticket IDs, namespace prefixes, user lists, etc.) |
...
All policies use the `package agentbox` package. The proxy queries `/v1/data/agentbox/allow`.

**Deny overrides allow.** The top-level `allow` rule in `main.rego` implements a deny-overrides-allow pattern: if any `deny` rule matches, the request is blocked regardless of any `allow` rule. This lets `agentbox deny` override preset and user-authored allow rules without editing their files.
...
Rules in `managed.rego` and `services.rego` contribute to `_allow_rules` and `deny` via Rego's incremental definition — multiple files can define rules for the same set, and OPA unions them:

```rego
# policies/services.rego (user-authored)
package agentbox

import rego.v1

# Contributes to _allow_rules in main.rego
_allow_rules contains true if {
...
```rego
# policies/managed.rego (auto-generated by CLI)
package agentbox

import rego.v1

# agentbox allow adds _allow_rules entries
_allow_rules contains true if { input.request.host == "docs.python.org" }

# agentbox deny adds deny entries — these override allow from any file
deny contains input.request.host if { input.request.host == "pypi.org" }
```

In the example above, `pypi.org` is allowed by the preset in `main.rego` but denied by `managed.rego`. The deny wins — `agentbox deny pypi.org` works without editing any preset file.
...
For better debugging and agent feedback, policies can optionally populate a `denial_reasons` set. The `deny` set (used by the deny-overrides-allow pattern in Section 5.3) already provides one source of denial reasons. Additional context-specific reasons can be added:
...
denial_reasons contains reason if {
    ...
    not input.request.body.fields.issue_key in data.allowed_tickets
    reason := sprintf("Jira access restricted to allowed tickets, got %s", [input.request.body.fields.issue_key])
}
```
...
### 7.1 Preset Allowlist and Deny Override
...
The preset allowlist in `main.rego` (see Section 5.3) defines default rules using `_preset_allow`. Users can override any preset rule using `agentbox deny <host>`, which adds a `deny` entry to `managed.rego` — no file editing needed.

```rego
# Example: user runs "agentbox deny pypi.org"
# This adds to managed.rego:
deny contains input.request.host if { input.request.host == "pypi.org" }

# The deny entry overrides the _preset_allow rule in main.rego that allows pypi.org.
# All other preset rules remain active.
```
...
### 7.7 Managed Rules — `agentbox allow/deny`
...
# agentbox allow <host> adds _allow_rules entries
_allow_rules contains true if { input.request.host == "docs.python.org" }
_allow_rules contains true if { input.request.host == "registry.npmjs.org" }

# agentbox deny <host> adds deny entries (overrides allow from any file)
deny contains input.request.host if { input.request.host == "pypi.org" }
```
...
Currently writes to `extra_request_policy` in `proxy.yaml` and triggers a proxy reload. Updated to write a Rego `_allow_rules` entry to `managed.rego` in the policy directory.
...
2. Append an `_allow_rules contains true if { ... }` rule to `managed.rego`.
...
Writes a `deny contains input.request.host if { ... }` rule to `managed.rego`. Because deny overrides allow (see Section 5.3), this blocks the host regardless of which file allows it — preset rules in `main.rego`, user rules in `services.rego`, or other allow rules in `managed.rego` itself.

Running `agentbox allow <host>` after a deny removes the deny entry and adds an allow entry.
...
The OPA server is on `opa-net`, a dedicated internal network. The agent container is on `agent-net` only and is **not** attached to `opa-net`. Network isolation is enforced by Podman's network namespace — no IP route or DNS resolution exists between the agent and OPA. This cannot be bypassed by the agent even with root access (unsetting `HTTP_PROXY`, writing custom scripts, etc.). The agent cannot:

- Query OPA to discover what policies are in effect (no network route to OPA).
- Modify OPA policies (policy files are bind-mounted read-only; agent has no filesystem access to the OPA container).
- Bypass OPA by connecting directly to external services (`agent-net` has no internet route; `proxy-net` and `opa-net` are only attached to the proxy).
